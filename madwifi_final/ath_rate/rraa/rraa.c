...
#define is_min_rate(ni) (ni->ni_txrate == 0)
	...
	if ( old_rate != ni->ni_txrate ){
	  ...
	  DPRINTF(sc, "rraa: " MAC_FMT " : ", MAC_ADDR(ni->ni_macaddr));  
	  DPRINTF(sc, 
...
ath_rate_attach(struct ath_softc *sc)
{
	...
	asc = kmalloc(sizeof(struct rraa_softc), GFP_ATOMIC);
	if (asc == NULL) {
...
{
	struct rraa_softc *asc = (struct rraa_softc *)arc;
	kfree(asc);
	_MOD_DEC_USE(THIS_MODULE);
}
...
static struct ieee80211_rate_ops ath_rate_ops = {
	.ratectl_id = IEEE80211_RATE_RRAA,
...
module_init(init_ath_rate_rraa_softc);

static void __exit
exit_ath_rate_rraa_softc(void)
{
	if (ath_sysctl_header != NULL)
		unregister_sysctl_table(ath_sysctl_header);
	ieee80211_rate_unregister(&ath_rate_ops);
}
module_exit(exit_ath_rate_rraa_softc);
