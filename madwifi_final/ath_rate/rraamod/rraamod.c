...
ath_rate_node_init(struct ath_softc *sc, struct ath_node *an)
{
  ...
  struct rraa_mod_node *rraa = ATH_NODE_RRAA_MOD(an);
  /* initialize rate */
  ...
  if (first_time){
    total_num_packets = 0;
      ...
      rraa_map[1].ewnd = 12;
      rraa_map[2].ewnd = 12;
      rraa_map[3].ewnd = 12; // 6Mbps
      rraa_map[4].ewnd = 20; // 9Mbps
      rraa_map[5].ewnd = 30; // 11Mbps
      rraa_map[6].ewnd = 40; // 12Mbps
      rraa_map[7].ewnd = 40; // 18Mbps
      rraa_map[8].ewnd = 50; // 24Mbps
      rraa_map[9].ewnd = 50;  // 36Mbps
      rraa_map[10].ewnd = 80;// 48Mbps
      rraa_map[11].ewnd = 80;// 54Mbps
		 ...
		 size_t frame_size, u_int8_t rix, struct ieee80211_mrr *mrr)
{
  	...
  	//struct rraa_node *rraa = ATH_NODE_RRAA(an);
...
ath_rate_update(struct ath_softc *sc, struct ieee80211_node *ni, int rate)
{
	...
	KASSERT(rt != NULL, ("rraa_mod: no rate table, mode %u", sc->sc_curmode));
...
#define	RATE(_ix)	(ni->ni_rates.rs_rates[(_ix)] & IEEE80211_RATE_VAL)
	...
	KASSERT(ni->ni_rates.rs_nrates > 0, ("rraa_mod: no rates"));
	if (vap->iv_fixed_rate == IEEE80211_FIXED_RATE_NONE) {
		...
		if (sc->sc_curmode != IEEE80211_MODE_11B) {
			...
			KASSERT(srate >= 0, ("rraa_mod: bogus rate set"));
		}
		#endif 
	} else {
		/*
		...
		KASSERT(srate >= 0,
			("rraa_mod: fixed rate %d not in rate set", vap->iv_fixed_rate));
	}
...
#define is_min_rate(ni) (ni->ni_txrate == 0)
	...
	struct rraa_mod_node *rraa = ATH_NODE_RRAA_MOD(ATH_NODE(ni));
...
#if 0
	DPRINTF (sc, "rraa_mod: tx_complete: %d tx_fail: %d tx_retry: %d tx_loss_ratio: %d\n",
		 rraa->rraa_tx_complete,
		 rraa->rraa_tx_fail,
		 rraa->rraa_tx_retry,
		 rraa->rraa_loss_ratio);	
	...
	DPRINTF(sc, "rraa_mod: " MAC_FMT " : ", MAC_ADDR(ni->ni_macaddr));  
...
ath_rate_attach(struct ath_softc *sc)
{
  ...
  struct rraa_mod_softc *asc;
  
	if (DEBUG){
	  printk("rraa_mod: this is the new rraa_mod module!\n");
	}

	_MOD_INC_USE(THIS_MODULE, return NULL);
	asc = kmalloc(sizeof(struct rraa_mod_softc), GFP_ATOMIC);
	if (asc == NULL) {
		_MOD_DEC_USE(THIS_MODULE);
		return NULL;
	}
	asc->arc.arc_space = sizeof(struct rraa_mod_node);
...
{
  struct rraa_mod_softc *asc = (struct rraa_mod_softc *)arc;
  kfree(asc);
  _MOD_DEC_USE(THIS_MODULE);
}
...
static struct ieee80211_rate_ops ath_rate_ops = {
	.ratectl_id = IEEE80211_RATE_RRAAMOD,
...
static int __init
init_ath_rate_rraa_mod_softc(void)
{
...
module_init(init_ath_rate_rraa_mod_softc);

static void __exit
exit_ath_rate_rraa_mod_softc(void)
{
	if (ath_sysctl_header != NULL)
		unregister_sysctl_table(ath_sysctl_header);
	ieee80211_rate_unregister(&ath_rate_ops);
}
module_exit(exit_ath_rate_rraa_mod_softc);
