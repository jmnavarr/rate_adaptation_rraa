...
#ifndef _DEV_ATH_RATE_RRAAMOD_H
#define _DEV_ATH_RATE_RRAAMOD_H
...
/* per-device state */
struct rraa_mod_softc {
	struct ath_ratectrl arc;		/* base state */
};

/* per-node state */
struct rraa_mod_node {
...
#define	ATH_NODE_AMRR(an)	((struct rraa_mod_node *)&an[1])
#define	ATH_NODE_RRAA_MOD(an)	((struct rraa_mod_node *)&an[1])
