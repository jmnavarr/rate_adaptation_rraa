...
ath_rate_node_init(struct ath_softc *sc, struct ath_node *an)
{
  ...
  struct rraa_c1_node *rraa = ATH_NODE_C1_RRAA(an);
    /* initialize rate */
		 ...
		 size_t frame_size, u_int8_t rix, struct ieee80211_mrr *mrr)
{
  	...
  	//struct rraa_c1_node *rraa = ATH_NODE_RRAA(an);
...
#define is_min_rate(ni) (ni->ni_txrate == 0)
	...
	struct rraa_c1_node *rraa = ATH_NODE_C1_RRAA(ATH_NODE(ni));
	...
	if ( old_rate != ni->ni_txrate ){
	  ...
	  // Only output statistics when rates change (022809)
	  DPRINTF(sc, "rraac1: " MAC_FMT " : ", MAC_ADDR(ni->ni_macaddr));  
	  DPRINTF(sc, 
...
ath_rate_attach(struct ath_softc *sc)
{
	...
	struct rraa_c1_softc *asc;

	if (DEBUG){
		printk("rraa: this is the new rraa module!\n");
	}

	_MOD_INC_USE(THIS_MODULE, return NULL);
	asc = kmalloc(sizeof(struct rraa_c1_softc), GFP_ATOMIC);
	if (asc == NULL) {
		_MOD_DEC_USE(THIS_MODULE);
		return NULL;
	}
	asc->arc.arc_space = sizeof(struct rraa_c1_node);
...
{
	struct rraa_c1_softc *asc = (struct rraa_c1_softc *)arc;
	kfree(asc);
	_MOD_DEC_USE(THIS_MODULE);
}
...
static struct ieee80211_rate_ops ath_rate_ops = {
	.ratectl_id = IEEE80211_RATE_RRAAC1,
...
static int __init
init_ath_rate_rraa_c1_softc(void)
{
...
module_init(init_ath_rate_rraa_c1_softc);

static void __exit
exit_ath_rate_rraa_c1_softc(void)
{
	if (ath_sysctl_header != NULL)
		unregister_sysctl_table(ath_sysctl_header);
	ieee80211_rate_unregister(&ath_rate_ops);
}
module_exit(exit_ath_rate_rraa_c1_softc);
