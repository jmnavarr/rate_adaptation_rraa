...
ath_rate_node_init(struct ath_softc *sc, struct ath_node *an)
{
  ...
  if (first_time){
    total_num_packets = 0;
      ...
      /* initialize PORI - CASE 3*/
      rraa_map[0].pori = 63;
      rraa_map[1].pori = 63;
      rraa_map[2].pori = 63;
      rraa_map[3].pori = 63; // 6Mbps
      rraa_map[4].pori = 18; // 9Mbps
      rraa_map[5].pori = 20; // 11Mbps
      rraa_map[6].pori = 24; // 12Mbps
      rraa_map[7].pori = 16; // 18Mbps
      rraa_map[8].pori = 21; // 24Mbps
      rraa_map[9].pori = 15;  // 36Mbps
      rraa_map[10].pori = 6;// 48Mbps
      rraa_map[11].pori = 0;// 54Mbps
      
      /* initialize PMTL - CASE 3 */
      rraa_map[0].pmtl = 39;
      rraa_map[1].pmtl = 39;
      rraa_map[2].pmtl = 39;
...
#define is_min_rate(ni) (ni->ni_txrate == 0)
	...
	struct rraa_node *rraa = ATH_NODE_RRAA(ATH_NODE(ni));
	...
	if ( old_rate != ni->ni_txrate ){
	  ...
	  DPRINTF(sc, "rraac3: " MAC_FMT " : ", MAC_ADDR(ni->ni_macaddr));  
	  DPRINTF(sc, 
...
ath_rate_attach(struct ath_softc *sc)
{
	...
	struct rraa_c3_softc *asc;

	if (DEBUG){
		printk("rraac3: this is the new rraa module!\n");
	}

	_MOD_INC_USE(THIS_MODULE, return NULL);
	asc = kmalloc(sizeof(struct rraa_c3_softc), GFP_ATOMIC);
	if (asc == NULL) {
		_MOD_DEC_USE(THIS_MODULE);
		return NULL;
	}
	asc->arc.arc_space = sizeof(struct rraa_node);
...
{
	struct rraa_c3_softc *asc = (struct rraa_c3_softc *)arc;
	kfree(asc);
	_MOD_DEC_USE(THIS_MODULE);
}
...
static struct ieee80211_rate_ops ath_rate_ops = {
	.ratectl_id = IEEE80211_RATE_RRAAC3,
...
static int __init
init_ath_rate_rraa_c3_softc(void)
{
...
module_init(init_ath_rate_rraa_c3_softc);

static void __exit
exit_ath_rate_rraa_c3_softc(void)
{
	if (ath_sysctl_header != NULL)
		unregister_sysctl_table(ath_sysctl_header);
	ieee80211_rate_unregister(&ath_rate_ops);
}
module_exit(exit_ath_rate_rraa_c3_softc);
