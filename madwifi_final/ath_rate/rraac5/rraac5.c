...
ath_rate_node_init(struct ath_softc *sc, struct ath_node *an)
{
	...
	struct rraa_c5_node *rraa = ATH_NODE_C5_RRAA(an);
    /* initialize rate */
...
#define is_min_rate(ni) (ni->ni_txrate == 0)
	...
	struct rraa_c5_node *rraa = ATH_NODE_C5_RRAA(ATH_NODE(ni));
...
ath_rate_attach(struct ath_softc *sc)
{
	...
	struct rraa_c5_softc *asc;

	if (DEBUG){
		printk("rraac5: this is the new rraa module!\n");
	}

	_MOD_INC_USE(THIS_MODULE, return NULL);
	asc = kmalloc(sizeof(struct rraa_c5_softc), GFP_ATOMIC);
	if (asc == NULL) {
		_MOD_DEC_USE(THIS_MODULE);
		return NULL;
	}
	asc->arc.arc_space = sizeof(struct rraa_c5_node);
...
{
	struct rraa_c5_softc *asc = (struct rraa_c5_softc *)arc;
	kfree(asc);
	_MOD_DEC_USE(THIS_MODULE);
}
...
static struct ieee80211_rate_ops ath_rate_ops = {
	.ratectl_id = IEEE80211_RATE_RRAAC5,
...
static int __init
init_ath_rate_rraa_c5_softc(void)
{
...
module_init(init_ath_rate_rraa_c5_softc);

static void __exit
exit_ath_rate_rraa_c5_softc(void)
{
	if (ath_sysctl_header != NULL)
		unregister_sysctl_table(ath_sysctl_header);
	ieee80211_rate_unregister(&ath_rate_ops);
}
module_exit(exit_ath_rate_rraa_c5_softc);
