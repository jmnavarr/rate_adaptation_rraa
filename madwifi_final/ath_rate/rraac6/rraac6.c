...
ath_rate_node_init(struct ath_softc *sc, struct ath_node *an)
{
	...
	struct rraa_c6_node *rraa = ATH_NODE_C6_RRAA(an);
    /* initialize rate */
  ...
  if (first_time){
    total_num_packets = 0;
      ...
      rraa_map[1].pori = 8;
      rraa_map[2].pori = 8;
      rraa_map[3].pori = 8; // 6Mbps
      rraa_map[4].pori = 2; // 9Mbps
      rraa_map[5].pori = 2; // 11Mbps
      rraa_map[6].pori = 2; // 12Mbps
      rraa_map[7].pori = 3; // 18Mbps
      rraa_map[8].pori = 2; // 24Mbps
      rraa_map[9].pori = 2;  // 36Mbps
      rraa_map[10].pori = 1;// 48Mbps
      rraa_map[11].pori = 0;// 54Mbps
      
      // initialize PMTL - CASE 6
      rraa_map[0].pmtl = 72;
      rraa_map[1].pmtl = 72;
      rraa_map[2].pmtl = 72;
      rraa_map[3].pmtl = 72; // 6Mbps
      rraa_map[4].pmtl = 72; // 9Mbps
      rraa_map[5].pmtl = 59; // 11Mbps
      rraa_map[6].pmtl = 54; // 12Mbps
      rraa_map[7].pmtl = 68; // 18Mbps
      rraa_map[8].pmtl = 50; // 24Mbps
      rraa_map[9].pmtl = 63;  // 36Mbps
      rraa_map[10].pmtl = 43;// 48Mbps
      rraa_map[11].pmtl = 17;// 54Mbps
      ...
      rraa_map[1].pori = 38;
      rraa_map[2].pori = 38;
      rraa_map[3].pori = 38; // 6Mbps
      rraa_map[4].pori = 11; // 9Mbps
      rraa_map[5].pori = 12; // 11Mbps
      rraa_map[6].pori = 14; // 12Mbps
      rraa_map[7].pori = 10; // 18Mbps
      rraa_map[8].pori = 13; // 24Mbps
      rraa_map[9].pori = 9;  // 36Mbps
      rraa_map[10].pori = 4;// 48Mbps
      rraa_map[11].pori = 0;// 54Mbps
      ...
      rraa_map[1].pmtl = 49;
      rraa_map[2].pmtl = 49;
      rraa_map[3].pmtl = 49; // 6Mbps
      rraa_map[4].pmtl = 49; // 9Mbps
      rraa_map[5].pmtl = 40; // 11Mbps
      rraa_map[6].pmtl = 36; // 12Mbps
      rraa_map[7].pmtl = 46; // 18Mbps
      rraa_map[8].pmtl = 34; // 24Mbps
      rraa_map[9].pmtl = 43;  // 36Mbps
      rraa_map[10].pmtl = 29;// 48Mbps
      rraa_map[11].pmtl = 11;// 54Mbps
      ...
      rraa_map[1].pmtl = 39;
      rraa_map[2].pmtl = 39;
      rraa_map[3].pmtl = 39; // 6Mbps
      rraa_map[4].pmtl = 39; // 9Mbps
      rraa_map[5].pmtl = 32; // 11Mbps
      rraa_map[6].pmtl = 29; // 12Mbps
      rraa_map[7].pmtl = 37; // 18Mbps
      rraa_map[8].pmtl = 27; // 24Mbps
      rraa_map[9].pmtl = 34;  // 36Mbps
      rraa_map[10].pmtl = 23;// 48Mbps
      rraa_map[11].pmtl = 9;// 54Mbps
      ...
      struct ath_node *an, const struct ath_buf *bf)
{
  ...
  // control on rts/cts option Madwifi
  struct ieee80211vap *va = ni->ni_vap;
  
  // !RTSon and !Success
  if ( ( va->iv_rtsthreshold == IEEE80211_RTS_MAX ) && !(ts->ts_status == 0))
    {
      printk("rraac6: rts disabled %d\n", va->iv_rtsthreshold );
      rraa->rts_wnd++;
      rraa->rts_counter = rraa->rts_wnd ;
    }
  // RTSon xor Success
  else if ( !( va->iv_rtsthreshold == IEEE80211_RTS_MAX ) ^ 
	    (ts->ts_status == 0) )
    {
      if ( rraa->rts_wnd > 0 ){
	rraa->rts_wnd = rraa->rts_wnd/2;
      }
      
      if ( rraa->rts_wnd <= 0 ){
	va->iv_rtsthreshold = IEEE80211_RTS_MAX ;
	printk("rraac6: disabling rts %d\n", va->iv_rtsthreshold );
      }

      printk("rraac6: rts window %d\n", rraa->rts_wnd );
      rraa->rts_counter = rraa->rts_wnd ;
    }
  
  // RTScounter > 0
  if ( rraa->rts_counter > 0 ){
    // Default value = 2B
    va->iv_rtsthreshold = 2;
    printk("rraac6: rts enabled %d\n", va->iv_rtsthreshold );
    printk("rraac6: rts counter %d\n", rraa->rraa_counter );
    rraa->rts_counter --;
  }
		     ...
		     struct ath_node *an, const struct ath_buf *bf)
{
  ...
  //va->iv_rtsthreshold = 6 ;
  //printk("rraac6: ni->ni_vap rts = %d\n", va->iv_rtsthreshold );
  ...
  // RTSoff or Success
  if ( (va->iv_rtsthreshold == 2346) || (ts->ts_status == 0))
   {
...
#define is_min_rate(ni) (ni->ni_txrate == 0)
	...
	struct rraa_c6_node *rraa = ATH_NODE_C6_RRAA(ATH_NODE(ni));
...
#if 0
	DPRINTF (sc, "rraa: tx_complete: %d tx_fail: %d tx_retry: %d tx_loss_ratio: %d\n",
		 rraa->rraa_tx_complete,
		 rraa->rraa_tx_fail,
		 rraa->rraa_tx_retry,
		 rraa->rraa_loss_ratio);	
	...
	if ( old_rate != ni->ni_txrate ){
	  ...
	  DPRINTF(sc, "rraac6: " MAC_FMT " : ", MAC_ADDR(ni->ni_macaddr));  
	  DPRINTF(sc, 
...
ath_rate_attach(struct ath_softc *sc)
{
	...
	struct rraa_c6_softc *asc;

	if (DEBUG){
		printk("rraac6: this is the new rraa module!\n");
	}

	_MOD_INC_USE(THIS_MODULE, return NULL);
	asc = kmalloc(sizeof(struct rraa_c6_softc), GFP_ATOMIC);
	if (asc == NULL) {
		_MOD_DEC_USE(THIS_MODULE);
		return NULL;
	}
	asc->arc.arc_space = sizeof(struct rraa_c6_node);
...
{
	struct rraa_c6_softc *asc = (struct rraa_c6_softc *)arc;
	kfree(asc);
	_MOD_DEC_USE(THIS_MODULE);
}
...
static struct ieee80211_rate_ops ath_rate_ops = {
	.ratectl_id = IEEE80211_RATE_RRAAC6,
...
static int __init
init_ath_rate_rraa_c6_softc(void)
{
...
module_init(init_ath_rate_rraa_c6_softc);

static void __exit
exit_ath_rate_rraa_c6_softc(void)
{
	if (ath_sysctl_header != NULL)
		unregister_sysctl_table(ath_sysctl_header);
	ieee80211_rate_unregister(&ath_rate_ops);
}
module_exit(exit_ath_rate_rraa_c6_softc);
